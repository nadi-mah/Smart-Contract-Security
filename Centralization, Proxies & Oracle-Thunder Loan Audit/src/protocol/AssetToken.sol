...
contract AssetToken is ERC20 {
    ...
    constructor(
        address thunderLoan,
        IERC20 underlying, //e the token which deposit to the system
    ...
    // only the thunderLoan contract can mint asset token
    ...
    function transferUnderlyingTo(
        ...
        // @follow up, can a weird erc20 brake the logic?
        i_underlying.safeTransfer(to, amount);
    }
    // e responsible for updating the exchange rate -> underlying
    function updateExchangeRate(uint256 fee) external onlyThunderLoan {
        ...
        // q what happen if the totalSupply() is 0?

        // @audit gas, too many storage reads -> storage as a memory
